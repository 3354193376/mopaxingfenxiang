...
$.ajax({
  ...
  data: {
    key: 'JFBZ-X6QCB-VR5U6-J77SN-PTNGJ-NGBIA',  // 这里要写你的KEY!!!
    output: 'jsonp',
  },
  dataType: 'jsonp',
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by Fomalhaut🥝 %c 你正在访问Mopaxing🥝の小家",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| mopaxing🥝") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('Mopaxing🥝的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
